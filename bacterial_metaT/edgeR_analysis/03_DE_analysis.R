...
targets <- list(
  bacteria = file.path(base_path, "edger_dge_bacteria.rds"),
  diatom   = file.path(base_path, "edger_dge_diatom.rds")
)

# Output directory
outdir <- file.path(base_path, "DE_edgeR_manualfilt_QL")
...
logcpm2 <- cpm(y2, log=TRUE, prior.count=1, normalized.lib.sizes=TRUE)
  ...
  # ---- design (no intercept) ----
  design <- model.matrix(~ 0 + condition, data = y2$samples)
  ...
  logcpm <- cpm(y2, log = TRUE, prior.count = 1, normalized.lib.sizes = TRUE)

  saveRDS(y2,   file.path(outdir, paste0(dataset_name, "_DGE_filtered_norm.rds")))
  ...
  barplot(y2$samples$lib.size / 1e6, las = 2,
          names.arg = y2$samples$condition,
          main = paste0(dataset_name, ": raw lib.size (M)"),
          ylab = "Millions")

  eff_lib <- (y2$samples$lib.size * y2$samples$norm.factors) / 1e6
  barplot(eff_lib, las = 2,
          names.arg = y2$samples$condition,
          main = paste0(dataset_name, ": effective lib.size (M)"),
          ylab = "Millions")

  plotMDS(y2, labels = y2$samples$condition,
          col = as.numeric(y2$samples$condition),
  ...
  message(dataset_name, ": done. Kept ", nrow(y2), " features after filtering.")
  invisible(res_list)
}
...
base_path <- "/scratch/timtd/transcriptomes/trimmed/salmon_quant_clst/edger_objects"
de_path   <- file.path(base_path, "DE_edgeR_manualfilt_QL")
...
res_annot <- res %>%
  left_join(annot, by=c("feature_id"="transcript_id"))
